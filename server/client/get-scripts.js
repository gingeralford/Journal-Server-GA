...
function displayMine() {
    ...
    .then(function (response) {
        ...
        for(i = 0; i = display.childNodes.length; i++) {
            ...
            //removes all the old displayed journals from the page, before displaying new ones
        }
        
        //handles the case if there are no journals to display
...
function displayAll() {
    ...
    // const accessToken = localStorage.getItem('SessionToken')
    //I GUESS NO ACCESS TOKEN NEEDED TO SEE ALL?
...
function displayByTitle() {
    ...
    fetch(`http://localhost:3000/journal/${title}`, {
        method: 'GET',
        headers: new Headers({
            'Content-Type': 'application/json',
            // 'Authorization': accessToken 
            //I GUESS NO ACCESS TOKEN NEEDED TO SEE BY TITLE?
        })
    })
    .then(response => response.json())
    .catch(function (error) {
